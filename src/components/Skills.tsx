...
export default function Skills() {
  ...
  useEffect(() => {
    ...
    const barObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          gsap.to(el, { width: level + '%', duration: 1.1, ease: 'power3.out' })
    ...
    return () => { ctx.revert(); barObserver.disconnect() }
  }, [])
    ...
    <section id="skills" ref={sectionRef} className="section" style={{ borderTop: '1px solid var(--border)' }}>
      <div className="container">
        <div className="skills-heading" style={{ marginBottom: 56 }}>
          <div className="eyebrow" style={{ marginBottom: 12 }}>02 — Skills</div>
                ...
                {skills.map(({ name: skill, level }) => (
                  <div key={skill}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      ...
                      <span style={{ fontFamily: 'Geist Mono, monospace', fontSize: 11, color: 'var(--text-faint)' }}>{level}%</span>
                    </div>
