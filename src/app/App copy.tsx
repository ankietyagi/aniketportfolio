...
// ─── Data ─────────────────────────────────────────────────────────────────────
const NAV = ["Home","About","Experience","Skills","Projects","Services","Contact"];
        ...
        <motion.div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-20"
          ...
          <Mag>
            <motion.button
              ...
              className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl text-[0.875rem] font-semibold text-white"
              style={{ fontFamily:SF, background:`linear-gradient(135deg,${BLUE},${PURPLE})`,
...
function Projects({ theme }: { theme: Theme }) {
  ...
  return (
    <Sec id="projects" theme={theme} alt>
      ...
      <div className="max-w-6xl mx-auto px-8">
        <SH eye="// Portfolio" title="Featured Projects"
          sub="A curated selection of work — shipped, scaled, and live." theme={theme}/>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {PROJECTS.map((p, i) => (
            <motion.div key={i} ref={el => { refs.current[i] = el; }}
              ...
              variants={fadeUp} initial="hidden" whileInView="visible"
              viewport={{ once:true, margin:"-60px" }} transition={tx(i*0.07)}
              onHoverStart={() => setH(i)} onHoverEnd={() => { setH(null); reset(i); }}
              onMouseMove={e => tilt(e,i)} onMouseLeave={() => reset(i)}
              whileHover={{ y:-6, boxShadow:d
                ? "0 20px 56px rgba(0,0,0,0.4), 0 0 0 1px rgba(59,130,246,0.22)"
                : "0 20px 56px rgba(59,130,246,0.13), 0 0 0 1px rgba(255,255,255,0.98)" }}
              data-h>
              <div className="relative h-44 overflow-hidden"
                style={{ background:d?"#0a0f1e":"#dbeafe" }}>
                <motion.img src={p.img} alt={p.title} className="w-full h-full object-cover"
                  animate={{ scale:hov===i?1.07:1 }} transition={{ duration:0.55 }}/>
                <div className="absolute inset-0" style={{ background:d
                  ? "linear-gradient(to bottom,transparent 30%,rgba(3,7,18,0.96))"
                  : "linear-gradient(to bottom,transparent 30%,rgba(240,244,252,0.97))" }}/>
                <motion.div className="absolute top-3 right-3 flex gap-2"
                  animate={{ opacity:hov===i?1:0, y:hov===i?0:-6 }} transition={{ duration:0.2 }}>
                  <a href="#" className="w-8 h-8 rounded-xl flex items-center justify-center"
                    style={{ background:"rgba(59,130,246,0.92)" }} onClick={e=>e.stopPropagation()}>
                    <FiExternalLink size={12} color="white"/>
                  </a>
                  <a href="#" className="w-8 h-8 rounded-xl flex items-center justify-center"
                    style={{ background:"rgba(0,0,0,0.44)", backdropFilter:"blur(8px)" }} onClick={e=>e.stopPropagation()}>
                    <FiGithub size={12} color="white"/>
                  </a>
                </motion.div>
              </div>
              <div className="p-5">
                <h3 className="font-bold text-[0.9375rem] mb-2 leading-snug"
                  style={{ fontFamily:DF, color:d?"#eef2ff":"#0c0f1d" }}>{p.title}</h3>
                <p className="text-xs leading-[1.72] mb-4"
                  style={{ fontFamily:SF, color:d?"rgba(238,242,255,0.46)":"rgba(12,15,29,0.5)" }}>{p.desc}</p>
                <div className="flex flex-wrap gap-1.5">
                  {p.tags.map(t => (
                    <span key={t} className="text-[0.7rem] px-2 py-0.5 rounded-lg"
                      style={{ fontFamily:MF, background:"rgba(59,130,246,0.08)", color:BLUE,
      ...
      <div className="max-w-6xl mx-auto px-8">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          <motion.div className="space-y-4" variants={fadeUp} initial="hidden" whileInView="visible"
            viewport={{ once:true }} transition={tx(0)}>
            {[
              { I:FiMail,   l:"Email",    v:"aniket@example.com", h:"mailto:aniket@example.com" },
              { I:FiPhone,  l:"Phone",    v:"+91 98765 43210",    h:"tel:+919876543210" },
              { I:FiMapPin, l:"Location", v:"New Delhi, India",   h:"#" },
            ].map(({ I, l, v, h }) => (
